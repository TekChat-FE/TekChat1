...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show confirm modal for new or existing session
      const userId = localStorage.getItem("userId");
      if (userId) {
        setCurrentUserId(userId);
  ...
  const handleConfirmContinue = () => {
    ...
    toast.success("Continuing with current session.", {
      position: "top-center",
      autoClose: 3000,
    });
    router.push("/roomlist");
  ...
  >
    <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
      ...
      <p className="text-base text-gray-700 mb-6 font-medium">
        You are currently signed in with the account above. Continue to proceed or switch to a different account.
      </p>
      ...
      <div className="flex flex-col gap-4">
        ...
        <button
          ...
          aria-label="Log out and switch account"
        >
          <ArrowRightOnRectangleIcon className="h-5 w-5" />
          Switch Account
        </button>
